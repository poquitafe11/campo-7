...
const allPhenologyFeatures = [
  {
    icon: <FilePlus2 className="h-8 w-8 text-primary" />,
    title: "Crear Registro",
    description: "Ingresa una nueva evaluación fenológica.",
    href: "/quality-control/phenology/create",
  },
  {
    icon: <Database className="h-8 w-8 text-primary" />,
    title: "Base de Registro",
    description: "Consulta el historial de todas las evaluaciones.",
    href: "/quality-control/phenology/database",
  },
  {
    icon: <BarChart3 className="h-8 w-8 text-primary" />,
    title: "Resumen Fenológico",
    description: "Visualiza reportes y proyecciones.",
    href: "/quality-control/phenology/summary",
  },
];

type Feature = (typeof allPhenologyFeatures)[0];
...
export default function PhenologyPage() {
    ...
    const visibleFeatures = useMemo(() => {
        ...
        if (profile.rol === 'Admin') {
            return allPhenologyFeatures;
        }
        if (!profile.permissions) {
            return [];
        }
        return allPhenologyFeatures.filter(feature => profile.permissions![feature.href]);
    }, [profile]);

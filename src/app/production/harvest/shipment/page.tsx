...
const allShipmentFeatures = [
  {
    icon: <FilePlus2 className="h-8 w-8 text-primary" />,
    title: "Registro",
    description: "Crea un nuevo registro de embarque.",
    href: "/production/harvest/shipment/register",
  },
  {
    icon: <Database className="h-8 w-8 text-primary" />,
    title: "Base de Datos",
    description: "Consulta el historial completo de embarques.",
    href: "/production/harvest/shipment/database",
  },
  {
    icon: <BarChart3 className="h-8 w-8 text-primary" />,
    title: "Resumen",
    description: "Visualiza reportes y estadísticas de embarques.",
    href: "/production/harvest/shipment/summary",
  },
   {
    icon: <TrendingUp className="h-8 w-8 text-primary" />,
    title: "Proyección",
    description: "Estima y proyecta los futuros embarques.",
    href: "/production/harvest/shipment/projection",
  },
];

type Feature = (typeof allShipmentFeatures)[0];
...
export default function ShipmentPage() {
  ...
  const visibleFeatures = useMemo(() => {
    ...
    if (profile.rol === 'Admin') {
      return allShipmentFeatures;
    }
    if (!profile.permissions) {
      return [];
    }
    return allShipmentFeatures.filter(feature => profile.permissions![feature.href]);
  }, [profile]);

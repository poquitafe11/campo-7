...
const allTardeFeatures = [
  {
    icon: <UserCheck className="h-8 w-8 text-primary" />,
    title: "Registro de Tarde",
    description: "Registra la asistencia del personal de la tarde.",
    href: "/tarde/register",
  },
  {
    icon: <Database className="h-8 w-8 text-primary" />,
    title: "Base de Datos",
    description: "Consulta el historial de registros de la tarde.",
    href: "/tarde/database",
  },
  {
    icon: <BarChart3 className="h-8 w-8 text-primary" />,
    title: "Resumen",
    description: "Visualiza reportes y estadísticas de la tarde.",
    href: "/tarde/summary",
  },
];

type Feature = (typeof allTardeFeatures)[0];
...
export default function TardePage() {
  ...
  useEffect(() => {
    setActions({ title: "Gestión de Tarde" });
    return () => setActions({});
  }, [setActions]);
  ...
  const visibleFeatures = useMemo(() => {
    ...
    if (profile.rol === 'Admin') {
      return allTardeFeatures;
    }
    if (!profile.permissions) {
      return [];
    }
    return allTardeFeatures.filter(feature => profile.permissions![feature.href]);
  }, [profile]);

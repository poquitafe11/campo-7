...
export function ResumenTablasAdicionales({ allRecords, allLotes, allLabors, selectedDate }: ResumenTablasProps) {
    ...
    const resumenPorLote = useMemo(() => {
        ...
        const sortedData = Object.values(data).sort((a, b) => {
            const codeA = String(a.codLabor || '').trim();
            const codeB = String(b.codLabor || '').trim();
            ...
            return (a.codLabor || '').localeCompare(b.codLabor || '');
        });
    ...
    if (recordsForSelectedDate.length === 0) return null;
